...
- **Manifest & cites → `unverifiable`.** `sources/MANIFEST.md` (generated by `assay -make-manifest
  sources/`) lists every held document by a canonical id — `hearing:<date>/<session>`,
  `submission:<N>`, `submission:<N>/attachment-<k>`, `qon:<org>/<date>`. A claims file may carry a 4th
  tab column, `cites`: the corpus-class documents a finding rests on. With `-manifest MANIFEST.md`, a
  claim citing a document **not** in the manifest is judged `unverifiable` **from code, no model call**
  — its reason names the missing document and its so-what is "fetch X". This is distinct from `absent`
  (the cited document is present and the claim is not found in it). `unverifiable` is its own Needs-you
  tier, ranked below the red ones. External datasets (a survey, an annual report) are not corpus-class
  documents and are not cited here — that is the evidence-grounding mode's concern.
...
- **A verdict needs a verified quote (code, not prompt).** Every verdict except `absent` must be
  backed by at least one quote that survives the grounding check. With none: `contradicted` (asserts
  the source says the opposite) becomes `absent`; `faithful`/`partial`/`overstated` (assert the source
  supports some version) become `unsupported` — no verdict, routed to the Needs-you tier for a human.
  `absent` needs no quote (it asserts nothing is there). Each downgrade is counted as a
  `no_quote_downgrade`. The rule is in code so a fluent but ungrounded judgment cannot talk its way to
  a verdict. Faithfulness verdicts are therefore `faithful` | `partial` | `overstated` | `absent` |
...
| `-manifest`    | `""`                        | MANIFEST.md of held docs; a claim citing a missing one is `unverifiable` (no model call). |
| `-make-manifest`| `""`                       | Scan this sources root, write `<root>/MANIFEST.md`, and exit. |
